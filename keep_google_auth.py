#!/usr/bin/env python3
"""
One-time OAuth authorization for Google Keep (headless/remote-friendly).
...
from config import KEEP_CREDS_FILE, KEEP_TOKEN_FILE, KEEP_SCOPES
...
def main():
    if not os.path.exists(KEEP_CREDS_FILE):
        print(f"Error: {KEEP_CREDS_FILE} not found.")
        sys.exit(1)

    with open(KEEP_CREDS_FILE) as f:
    ...
    if os.path.exists(KEEP_TOKEN_FILE):
        print(f"Token already exists: {KEEP_TOKEN_FILE}")
    ...
    print(f"\nToken saved to {KEEP_TOKEN_FILE}")
    print("Google Keep plugin is now ready to use.")
