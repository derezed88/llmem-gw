...
from config import CALENDAR_CREDS_FILE, CALENDAR_TOKEN_FILE, CALENDAR_SCOPES
...
def main():
    if not os.path.exists(CALENDAR_CREDS_FILE):
        print(f"Error: {CALENDAR_CREDS_FILE} not found.")
        sys.exit(1)

    with open(CALENDAR_CREDS_FILE) as f:
    ...
    if os.path.exists(CALENDAR_TOKEN_FILE):
        print(f"Token already exists: {CALENDAR_TOKEN_FILE}")
    ...
    scopes_str = " ".join(CALENDAR_SCOPES)
    auth_params = {
        "client_id": client_id,
    ...
    # Save in the same authorized-user format google-auth expects
    creds_data = {
    ...
    print(f"\nToken saved to {CALENDAR_TOKEN_FILE}")
    print("Google Calendar plugin is now ready to use.")
